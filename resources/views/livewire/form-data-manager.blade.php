<div class="max-w-7xl mx-auto p-6">
    ...
    @if(empty($code_id))
        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            <h2 class="text-xl font-semibold mb-4">Select a Code to Add Form Data</h2>
            @if($codes->isEmpty())
            ...
            @else
                <ul class="space-y-2">
                    @foreach($codes as $code)
                        <li>
                            <button wire:click="$set('code_id', {{ $code->id }})" class="text-blue-500 hover:underline">{{ $code->code_name }}</button>
                        </li>
                    @endforeach
                </ul>
            @endif
    ...
    @else
        <!-- Create Form -->
        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            <h2 class="text-xl font-semibold mb-4">Create Form Data for Code {{ $codes->firstWhere('id', $code_id)->code_name ?? 'Unknown' }}</h2>
